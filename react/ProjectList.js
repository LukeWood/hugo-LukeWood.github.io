...
{
		var clone = JSON.parse(JSON.stringify(to_remove));
		to_remove = [];
		clone.forEach(function(rem){
				for(var i = 0; i < currentLocs.length; i++){
						if(currentLocs[i].x == rem.x && currentLocs[i].y == rem.y){
	...
	tick: function()
	{
		...
		if(dec){
			...
			if(opac<=0){
				...
				to_remove.push({x:this.state.x, y:this.state.y});
				var pt = genNextPoint();
